...
cat > "$TMP_DIR/var/reports/run-29990101000000.json" <<EOF
{
  "timestamp": "$NOW",
  "processed_count": 0,
  "error_count": 2
...
for pattern in \
  ...
  '"code"[[:space:]]*:[[:space:]]*"last_run_errors"' \
  '"error_count"[[:space:]]*:[[:space:]]*2'
do
  if ! rg -q "$pattern" "$TMP_DIR/out.json"; then
    echo "expected service-status to mark fresh error report as attention: $pattern"
